...
///
/// Storage deletions (value → `Felt::ZERO`) are supported: callers must pre-fetch the
/// missing sibling preimages via `compute_missing_sibling_keys` in `storage_proofs.rs`.
